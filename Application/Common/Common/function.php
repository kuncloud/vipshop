...
function birth_msg($openid, $nickname){
	...
	$wxLogic->model_msg($openid, $data, 'birth');
}

function jf_msg($uid, $openid, $jf, $title){
	// 发送模板消息
// 	$openid = session('openid');
	$cid = session('cid');
	if ($openid){
		$wxLogic = new WxLogic();
		$total = M('Member')->where(array('id'=>$uid))->getField('jf');
		$data = array(
			'reason'=>$title,
			'type'=>$jf > 0 ? '增加' : '减少',
			'change'=>$jf,
		...
		$uri = "Record/index/cid/$cid/openid/$openid";
		$wxLogic->model_msg($openid, $data, 'jf', $uri);
	}
}

function dd_msg($id, $status, $openid){
	// 发送模板消息
// 	$openid = session('openid');
	$cid = session('cid');
	if ($openid){
		...
		$uri = "Order/detail/id/$id/cid/$cid/openid/$openid";
		$wxLogic->model_msg($openid, $data, 'dd', $uri);
	}
}
 ...
 * time : 2017-4-17下午10:32:12
 */
function pre_bak($data, $field='corver', $pre=''){
	if (!$data) return '';
	if (!$field) return '';
...
function Param($name, $default='', $filter='', $datas=''){
	$get = I("get.$name", '', $filter, $datas);
	$post = I("post.$name", '', $filter, $datas);
	$val = dv($get, $post);
	$val = dv($val, $default);
	return $val;
}
...
function timetostr($time, $format='Y-m-d H:i'){
	return $time ? date($format, $time) : '';
}

function img_pre(){
	return C('QINIU_CONFIG.IMG_URL');
}
