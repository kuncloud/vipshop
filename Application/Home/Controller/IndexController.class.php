...
class IndexController extends HomeBaseController{
	...
	public function index() {
		$this->loadAd();
		$info = M('Member')->where(array('id'=>UID))->field('jf,avatar,phone')->find();
		...
		$where = array(
			'cid'=>CID,
			'status'=>1,
			'jf'=>array('elt', $info['jf'])
		);
		$order = 'create_time desc';
		$recGoods = $modelGoods->where($where)->order($order)->limit(8)->select();
		$recGoods = pre($recGoods, 'corver');
		$this->assign('rec', $recGoods);
		...
		$order = 'praise desc';
		$hotGoods = $modelGoods->where($where)->order($order)->limit(8)->select();
		$hotGoods = pre($hotGoods, 'corver');
		$this->assign('hot', $hotGoods);
		$this->adminDisplay();
	}
	
	public function czd(){
		$phone = M('Member')->where(array('id'=>UID))->getField('phone');
