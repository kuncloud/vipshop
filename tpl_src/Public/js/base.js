 ...
 */
export function xbGoTo(url,second,message){
 ...
 */
export function xbRefresh(second,history){
 ...
 * @param  {string} url 连接
 */
export function xbNeedLogin(url){
	if(xbCheckLogin()){
		xbGoTo(url);
	}else{
		xbAlert('您需要登录');
 ...
 */
export function xbNeedConfirm(url,word){
 ...
 */
export function xbSetCookie(name,value,days) {
    if (days) {
        var date = new Date();
...
// 获取cookie
export function xbGetCookie(name) {
    var nameEQ = name + "=";
...
// 删除cookie
export function xbDeleteCookie(name) {
    xbSetCookie(name,"",-1);
}
